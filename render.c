...
void render_frame_background(struct swaylock_surface *surface, bool commit) {
  ...
  struct pool_buffer *buffer = get_next_buffer(state->shm, surface->buffers,
  ...
  if (surface->image && state->args.mode != BACKGROUND_MODE_SOLID_COLOR) {
    ...
    if (fade_is_complete(&surface->fade)) {
      if (!surface->scaled_image) {
        surface->scaled_image = scale_background_image(
            surface->image, state->args.mode, buffer_width, buffer_height);
      }
      render_background_image(cairo, surface->scaled_image, 1);
    } else {
      if (!surface->screencopy.scaled_image) {
        surface->screencopy.scaled_image = scale_background_image(
            surface->screencopy.original_image, state->args.mode, buffer_width,
            buffer_height);
      }
      ...
      render_background_image(cairo, surface->scaled_image,
                              surface->fade.alpha);
    }
  }
  cairo_restore(cairo);
...
void render_frame(struct swaylock_surface *surface) {
  ...
  if (state->args.indicator ||
    ...
    if (image) {
      ...
      cairo_arc(cairo, buffer_diameter * 0.5, buffer_diameter * 0.5, radius, 0,
                2 * M_PI);
